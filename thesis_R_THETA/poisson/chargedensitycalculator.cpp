...
ChargeDensityCalculator::ChargeDensityCalculator(DConstFieldVrVtheta coeffs) : m_quadrature(coeffs) {}

void ChargeDensityCalculator::operator()(DFieldRTheta rho, DConstFieldSpVrVthetaRTheta allfdistribu) const
{
    ...
    m_quadrature(
            ...
            rho,
            KOKKOS_LAMBDA(IdxRThetaVrVtheta idx) {
                Real sum = 0.0;
    ...
    if (last_kin_species != last_species) {
        ...
        ddc::parallel_for_each(
                ...
                Kokkos::DefaultExecutionSpace(),
                get_idx_range(rho),
                KOKKOS_LAMBDA(IdxRTheta irt) { rho(irt) += chargedens_adiabspecies; });
    }
