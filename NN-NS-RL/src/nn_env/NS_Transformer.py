...
class DSAttention(nn.Module):
    ...
    def forward(self, q : torch.Tensor, k : torch.Tensor, v : torch.Tensor, att_mask : torch.Tensor, tau : Optional[torch.Tensor]= None, delta : Optional[torch.Tensor] = None):
        ...
        #print("tau size, delta size", tau.size(), delta.size())
        #print("size of q , k , v ", q.size(), k.size(), v.size())
...
class NStransformer(nn.Module):
    def __init__(
        ...
        self.decoder_input = nn.Sequential(
            nn.Conv1d(in_channels=input_ctrl_dim+input_0D_dim, out_channels=feature_dim // 2, kernel_size= kernel_size, stride = 1, padding = padding),
            nn.BatchNorm1d(feature_dim//2),
            nn.ReLU(),
            nn.Conv1d(in_channels=feature_dim // 2, out_channels=feature_dim, kernel_size= kernel_size, stride = 1, padding = padding),
            nn.BatchNorm1d(feature_dim),
    ...
    def forward(self, x_0D : torch.Tensor, x_ctrl : torch.Tensor, target_0D : Optional[torch.Tensor] = None, target_ctrl : Optional[torch.Tensor] = None):
        ...
        # concat : (N, T, F1 + F2)
        x_ctrl_hist = x_ctrl[:, :x_0D.size(1), :]
        x = torch.concat([x_0D, x_ctrl_hist], axis = 2)
    ...
    def summary(self):
        # Dynamically detect if the model is on CPU or GPU
        device = next(self.parameters()).device
        
        # Dummy inputs for the historical sequence (Encoder)
        sample_0D = torch.zeros((1, self.input_seq_len, self.input_0D_dim)).to(device)
        sample_ctrl = torch.zeros((1, self.input_seq_len, self.input_ctrl_dim)).to(device)
        
        # Dummy inputs for the future prediction window (Decoder)
        target_0D = torch.zeros((1, self.output_pred_len, self.output_0D_dim)).to(device)
        target_ctrl = torch.zeros((1, self.output_pred_len, self.input_ctrl_dim)).to(device)
        
        # Pass all 4 arguments to the model summarizer
        summary(
            self, 
            sample_0D, 
            sample_ctrl, 
